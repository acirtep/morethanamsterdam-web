  ...
  return lines
    .map((line, lineNum) => {
      ...
      if (lat < 50.0 || lat > 54.0 || lon < 3.0 || lon > 8.0) {
        console.warn(
          `Coordinates out of Netherlands bounds at line ${lineNum + 2}`,
        );
        return null;
      }
      ...
      const sanitizedMunicipality = municipalitySk
        ? municipalitySk.replace(/[<>'"&]/g, "")
        : "";
      ...
      return {
        ...
        lat,
        lon,
        municipalitySk: sanitizedMunicipality,
      };
    })
    .filter(Boolean);
}
...
export async function ensureDuckDB() {
  ...
  async function makeBlobWorker(url) {
    ...
    if (!resp.ok)
      throw new Error(`Failed to fetch worker: ${url} (${resp.status})`);
    ...
    if (resp.ok) {
      ...
      if (contentType && contentType.includes("javascript")) {
        ...
        if (
          ...
          pthreadUrl = URL.createObjectURL(
...
export async function ensureServicesFileRegistered(dayOfWeek) {
  ...
  await duckdbDb.registerFileBuffer(
    "./data/train_stations.csv",
